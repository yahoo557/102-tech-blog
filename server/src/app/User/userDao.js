...
const selectUserEmail = async (connection, email) => {
  const selectUserEmailQuery = `SELECT email, nickname FROM userInfo WHERE email = $1 and Status = 'ONLINE';`;
  return await connection.query(selectUserEmailQuery, email);
}

// userId 회원 조회
const selectUserId = async (connection, userId) => {
  const selectUserIdQuery = `SELECT id, email, nickname FROM UserInfo WHERE id = $1 and Status = 'ONLINE';`;
  return await connection.query(selectUserIdQuery, userId);
}

// 유저 생성
const insertUserInfo = async (connection, insertUserInfoParams) => {
  const insertUserInfoQuery = `INSERT INTO UserInfo(email, password, nickname)VALUES ($1, $2, $3);`;
...
const selectUserPassword= async (connection, selectUserPasswordParams) => {
  const selectUserPasswordQuery = `SELECT email, nickname, password FROM UserInfo WHERE email = $1 AND password = $2;`;
...
const selectUserAccount = async (connection, email) => {
  const selectUserAccountQuery = `SELECT status, id FROM user WHERE email = $1;`;
